function File = ppGetFullPath(File)
% ppGetFullPath - Get absolute path of a file or folder [MEX]
% FullName = ppGetFullPath(Name)
% INPUT:
...
%   ppGetFullPath('File.Ext')         % ==>  'C:\Temp\File.Ext'
%   ppGetFullPath('..\File.Ext')      % ==>  'C:\File.Ext'
%   ppGetFullPath('..\..\File.Ext')   % ==>  'C:\File.Ext'
%   ppGetFullPath('.\File.Ext')       % ==>  'C:\Temp\File.Ext'
%   ppGetFullPath('*.txt')            % ==>  'C:\Temp\*.txt'
%   ppGetFullPath('..')               % ==>  'C:\'
%   ppGetFullPath('Folder\')          % ==>  'C:\Temp\Folder\'
%   ppGetFullPath('D:\A\..\B')        % ==>  'D:\B'
%   ppGetFullPath('\\Server\Folder\Sub\..\File.ext')
%                                   % ==>  '\\Server\Folder\File.ext'
%   ppGetFullPath({'..', 'new'})      % ==>  {'C:\', 'C:\Temp\new'}
%
% COMPILE: See ppGetFullPath.c
%   Run the unit-test uTest_ppGetFullPath after compiling.
%
...
% $UnitTest: uTest_ppGetFullPath $
% $File: Tools\GLFile\ppGetFullPath.m $
% History:
...
% - Mex calls Windows system function ppGetFullPath and is therefore much more
%   stable.
% - Mex is much faster.

% Disable this warning for the current Matlab session:
%   warning off JSimon:ppGetFullPath:NoMex
...
%warning('JSimon:ppGetFullPath:NoMex', ...
%  'ppGetFullPath: Using slow M instead of fast Mex.');
...
% Handle cell strings:
% NOTE: It is faster to create a function @cell\ppGetFullPath.m under Linux,
% but under Windows this would shadow the fast C-Mex.
if isa(File, 'cell')
   for iC = 1:numel(File)
      File{iC} = ppGetFullPath(File{iC});
   end
   return;
end
