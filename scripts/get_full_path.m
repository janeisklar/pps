...
% get_full_path - Get absolute path of a file or folder [MEX]
...
%   get_full_path('File.Ext')         % ==>  'C:\Temp\File.Ext'
%   get_full_path('..\File.Ext')      % ==>  'C:\File.Ext'
%   get_full_path('..\..\File.Ext')   % ==>  'C:\File.Ext'
%   get_full_path('.\File.Ext')       % ==>  'C:\Temp\File.Ext'
%   get_full_path('*.txt')            % ==>  'C:\Temp\*.txt'
%   get_full_path('..')               % ==>  'C:\'
%   get_full_path('Folder\')          % ==>  'C:\Temp\Folder\'
%   get_full_path('D:\A\..\B')        % ==>  'D:\B'
%   get_full_path('\\Server\Folder\Sub\..\File.ext')
%                                   % ==>  '\\Server\Folder\File.ext'
%   get_full_path({'..', 'new'})      % ==>  {'C:\', 'C:\Temp\new'}
%
% COMPILE: See get_full_path.c
%   Run the unit-test uTest_get_full_path after compiling.
%
...
% $UnitTest: uTest_get_full_path $
% $File: Tools\GLFile\get_full_path.m $
% History:
...
% - Mex calls Windows system function get_full_path and is therefore much more
%   stable.
% - Mex is much faster.

% Disable this warning for the current Matlab session:
%   warning off JSimon:get_full_path:NoMex
...
%warning('JSimon:get_full_path:NoMex', ...
%  'get_full_path: Using slow M instead of fast Mex.');
...
% Handle cell strings:
% NOTE: It is faster to create a function @cell\get_full_path.m under Linux,
% but under Windows this would shadow the fast C-Mex.
if isa(File, 'cell')
   for iC = 1:numel(File)
      File{iC} = get_full_path(File{iC});
   end
   return;
end
